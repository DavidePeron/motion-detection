...
def resize_input(X_train, Y_train, X_test, Y_test):
	...
	Y_train = np.reshape(Y_train, (np.shape(Y_train)[0], np.shape(Y_train)[1], np.shape(Y_train)[2], 1))
	X_test = np.reshape(X_test, (np.shape(X_test)[0], np.shape(X_test)[1], np.shape(X_test)[2], 1))
	Y_test = np.reshape(Y_test, (np.shape(Y_test)[0], np.shape(Y_test)[1], np.shape(Y_test)[2], 1))
